...
```mermaid
graph TD
    Root["AI 技术全景"] --- Foundation & Core & Application

    subgraph Foundation ["🔬 底层基座"]
        direction LR
        Science["1. 科学原理<br>(Science)"]
        Resource["2. 基础设施<br>(Resource)"]
    end

    subgraph Core ["⚙️ 核心能力"]
        direction LR
        ModelData["3. 数据与模型<br>(Data/Model)"]
        Engineering["4. 研发工程<br>(Engineering)"]
    end

    subgraph Application ["🚀 应用与治理"]
        direction LR
        Architecture["5. 应用架构<br>(Architecture)"]
        Governance["6. 安全与治理<br>(Governance)"]
    end
    ...
    ModelData --- DataParsing["数据解析 (Unstructured/LlamaParse)"]
    ...
    Architecture --- FineTune["Fine-tune (领域适配/LoRA)"]
...
*   **基础设施 (GPU/Vector DB)**：物理底座。它为模型训练与推理提供算力，同时通过向量数据库为 RAG 系统提供长效检索存储能力。
...
| **记忆 (Memory)** | 维持上下文状态，短期记忆 (对话窗口) + 长期记忆 (Vector DB) | Chroma、Milvus、Mem0 |
...
### 1.5 工程体系
现代 AI 研发衍生出了确保系统鲁棒性与边界可控的三大学派，它们是将 Demo 演进为企业级生产应用的保障。下图展示了从输入约束到质量审计的完整反馈闭环：
...
| v0.2 | 2026-04-24 | 修复标题层级、统一项目名拼写、优化导引段落可读性、修复链接与 Star 数据、增加 1.4 节分隔线、添加 TOC 兼容性注释 |
