...
timeline
    ...
    2012-2017   : 判别式深度学习时代
                : CNN 统治视觉领域 / 算力初爆发
                : 核心能力：特定感知域超越人类
    2017-2024   : 生成式大模型时代 (System-1)
                : Transformer 架构登场 / Scaling Law
                : 核心能力：基于概率生成的快思考
    2025-Present: 推理与自主思考时代 (System-2)
                : 推理侧算力扩展 (Test-Time Compute)
                : 核心能力：反思、纠错与多步逻辑深潜
```

从**人工写逻辑**，到**机器找特征**，再到**机器自主涌现逻辑**，AI 已不再是简单的“复读机”，而是正在进化为能够独立解决复杂问题的“数字脑力”。

#### 1.3.2 2025-2026：从“快思考”到“慢思考” (Reasoning Era)

在这一阶段，大模型的评价标准从单纯的参数规模 (Parameters) 转向了 **推理侧算力 (Test-Time Compute, TTC)**。

*   **System-1 (快思考)**：早期的 GPT/Llama 模型主要依赖大规模预训练产生的“直觉”，即根据上文概率预测下一个 Token。这类似于人类的潜意识反应，虽快但容易产生“幻觉”。
*   **System-2 (慢思考)**：以 **DeepSeek-R1** 和 **OpenAI o1** 为代表的新型模型，在回答前会进行漫长的“自我对话”。它们在内部展开多步 **思维链 (CoT)**，不断地自我反思、验证并纠正推理路径。

> **核心实质**：AI 的智力上限不再仅仅取决于训练时烧了多少钱，而取决于它在回答每一个具体问题时，你愿意给它多少“思考时间”。这种“推理侧扩展”标志着 AI 真正具备了解决科学发现、数学证明等高难度逻辑任务的能力。
...
#### 11.1.3 自主智能体
**核心本质**：具备决策能力，根据任务自主决定调用工具，实现逻辑闭环。
```python
...
### 11.9 管理与工程效能 (Manager's Field Guide)

> **资深架构师提示**：在 AI 时代，代码只是表象，**数据质量**与**评估闭环**才是系统的真正护城河。

#### 11.9.1 评估驱动开发 (EDD - AI 时代的 TDD)
...
#### 11.9.4 AI 研发成熟度模型 (Maturity Model)
